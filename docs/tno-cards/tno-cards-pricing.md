# TNO Cards Pricing

## Pricing Table

| TNO Category | EXPL Amount | Price (USD) | 1 EXPL (USD) | Price Decline % |
|--------------|--------------|-------------|---------------|-----------------|
| **Bronze** | 10,000 | $80 | $0.008 | 0.00% |
| **Silver** | 100,000 | $720 | $0.0072 | -10.00% |
| **Gold** | 1,000,000 | $5,760 | $0.00576 | -28.00% |
| **Diamond** | 10,000,000 | $40,320 | $0.00403 | -49.60% |

---

## TNO Cards

<div style="display: flex; gap: 20px; justify-content: center; align-items: center; margin: 30px 0; flex-wrap: wrap;">
  <figure style="text-align: center;">
    <img src="../../assets/images/bronze.png" alt="TNO Bronze" width="375" />
    <figcaption><em>TNO Bronze</em></figcaption>
  </figure>
  <figure style="text-align: center;">
    <img src="../../assets/images/silver.png" alt="TNO Silver" width="375" />
    <figcaption><em>TNO Silver</em></figcaption>
  </figure>
</div>

<div style="display: flex; gap: 20px; justify-content: center; align-items: center; margin: 30px 0; flex-wrap: wrap;">
  <figure style="text-align: center;">
    <img src="../../assets/images/gold.png" alt="TNO Gold" width="375" />
    <figcaption><em>TNO Gold</em></figcaption>
  </figure>
  <figure style="text-align: center;">
    <img src="../../assets/images/diamond.png" alt="TNO Diamond" width="375" />
    <figcaption><em>TNO Diamond</em></figcaption>
  </figure>
</div>

---

## Pricing Details

=== "Bronze"
    **TNO Bronze Card**
    
    - **EXPL Amount:** 10,000
    - **Price:** $80
    - **Price per EXPL:** $0.008
    - **Best for:** Entry-level participants
    
=== "Silver"
    **TNO Silver Card**
    
    - **EXPL Amount:** 100,000
    - **Price:** $720
    - **Price per EXPL:** $0.0072
    - **Discount:** 10% price decline vs Bronze
    - **Best for:** Active community members
    
=== "Gold"
    **TNO Gold Card**
    
    - **EXPL Amount:** 1,000,000
    - **Price:** $5,760
    - **Price per EXPL:** $0.00576
    - **Discount:** 28% price decline vs Bronze
    - **Best for:** Serious investors
    
=== "Diamond"
    **TNO Diamond Card**
    
    - **EXPL Amount:** 10,000,000
    - **Price:** $40,320
    - **Price per EXPL:** $0.00403
    - **Discount:** 49.6% price decline vs Bronze
    - **Best for:** Strategic partners and major stakeholders

---

!!! success "Next Section"
    The next page provides a step-by-step guide for minting TNO Cards
