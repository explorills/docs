# Tokenomics Introduction

!!! success "EXPL Token - Powered by EXPL Nodes"
    EXPL has a max supply of **55,783,940,100 tokens**. Distribution happens in two ways: through EXPL Nodes (77.57%) and direct mint channels (22.43%). Node operators mint tokens through on-chain contracts, which automatically flow to designated escrows, rewards, and founding contributors royalty. Direct channels let users mint based on engagement and activity.

**2026 Milestone**: EXPL ERC20 deployment with 10M EXPL minimum liquidity pool

---

## Key Points

✅ **Main Distribution**: EXPL Nodes mint tokens via on-chain contracts  
✅ **ERC20 Token**: Deploying on Ethereum in 2026  
✅ **Community-Driven**: Operated by Node owners worldwide  
✅ **Sustainable Model**: Long-term incentive alignment  
✅ **Transparent**: All operations on-chain and verifiable  

---

## Section Content

<div class="grid cards" markdown>

-   :material-chart-pie:{ .lg .middle } __General Overview__

    ---

    Understanding the EXPL distribution model and allocation strategy

    [:octicons-arrow-right-24: Learn More](general-overview.md)

-   :material-database:{ .lg .middle } __Supply & Allocation__

    ---

    Complete breakdown of max supply and allocation categories

    [:octicons-arrow-right-24: View Supply](supply.md)

</div>

---

## EXPL Nodes: The Distribution Engine

EXPL Nodes are the primary mechanism for EXPL distribution in the ONE ecosystem:

- Daily mint permissions through on-chain contracts
- Decentralized, community-operated infrastructure
- First batch: 12,000 Nodes operational in Q2 2026
- Main distribution begins after ERC20 deployment

[:octicons-arrow-right-24: Learn About EXPL Nodes](../nodes/introduction.md)

[:octicons-arrow-right-24: View 2026 Roadmap](../roadmap-2026.md)

---
